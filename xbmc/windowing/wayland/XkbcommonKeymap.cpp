...
static const std::map<xkb_keycode_t, XBMCKey> XkbDeadKeyXBMCMapping = {
...
std::unique_ptr<CXkbcommonKeymap> CXkbcommonContext::LocalizedKeymapFromString(
    std::string const& keymap, std::string const& locale)
{
  ...
  std::unique_ptr<xkb_compose_table, CXkbcommonKeymap::XkbComposeTableDeleter> xbkComposeTable{
  ...
  return std::unique_ptr<CXkbcommonKeymap>{
      new CXkbcommonKeymap(std::move(xkbKeymap), std::move(xbkComposeTable))};
}
...
KeyComposerStatus CXkbcommonKeymap::KeyComposerFeed(xkb_keycode_t code) const
{
  ...
  if (mapping != XkbDeadKeyXBMCMapping.end())
  {
    composerStatus.keysym = mapping->second;
  }
...
void CXkbcommonKeymap::KeyComposerFlush() const
{
...
std::uint32_t CXkbcommonKeymap::UnicodeCodepointForKeycode(xkb_keycode_t code) const
{
  ...
  {
    uint32_t keysym = xkb_compose_state_get_one_sym(m_composeState.get());
