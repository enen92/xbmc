...
///     @note Will return -1 if not able to convert the provided value to integer. **Example**: `Integer.ValueOf(some string)` will evaluate to -1
      ...
      case INTEGER_ODD:
        {
          auto getIntValue = [this, &item, &contextWindow](int infoNum) {
            int integer = 0;
            if (!GetInt(integer, infoNum, contextWindow, item))
            {
              std::string value;
          ...
          int integerLeft = getIntValue(info.GetData1());
          int integerRight = getIntValue(info.GetData2());

          // compare
          if (condition == INTEGER_IS_EQUAL)
            bReturn = integerLeft == integerRight;
