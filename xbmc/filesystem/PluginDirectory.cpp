...
CPluginDirectory::SCRIPT_EXECUTION_INFO
              CPluginDirectory::TriggerScriptExecution(const std::string& strPath, bool resume)
{
...
bool CPluginDirectory::ExecuteScriptAndWaitOnResult(const std::string& strPath, bool resume)
{
  auto scriptExecutionInfo = TriggerScriptExecution(strPath, resume);
  bool success = false;
...
void CPluginDirectory::WaitForScriptToFinish(SCRIPT_EXECUTION_INFO scriptExecutionInfo,
                                             int mSecs, bool bForceStop)
{
...
void CPluginDirectory::ForceStopRunningScript(SCRIPT_EXECUTION_INFO scriptExecutionInfo)
{
  if (scriptExecutionInfo.Id != -1 &&
      CScriptInvocationManager::GetInstance().IsRunning(scriptExecutionInfo.Id))
  {
    CLog::Log(LOGDEBUG, "%s- cancelling plugin %s (id=%d)",
              __FUNCTION__, m_addon->Name().c_str(), scriptExecutionInfo.Id);
