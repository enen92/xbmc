...
private:
  ...
  */
  void WaitForScriptToFinish(SCRIPT_EXECUTION_INFO scriptExecutionInfo, int mSecs, bool bForceStop);
...
/*!
* A public asynchronous action to get the result of a plugin path expansion. This class can be invoked from some sort of busyloader
 (e.g. the busydialog) or be executed synchronously instead through ExecuteSynchronously().
*/
...
public:
  ...
  /*!
  \brief Create a new AsyncGetPluginResultAction which can be later drived asynchronously or executed syncronously
  \param strPath - The complete path of the plugindirectory to retrieve
  \param resultItem - The item to be resolved (usually has the path of strPath)
  \param resume - If the plugin directory retrieval should be executed with resume arguments
   */
  AsyncGetPluginResultAction(const std::string& strPath, CFileItem& resultItem, bool resume);
  
  /*!
  \brief Check if the execution had success
   \return If the plugin directory retrieval had success  (plugin directory retrieved and not canceled)
  */
  bool ExecutionHadSuccess() const;
  
  /*!
  \brief Cancels the action async execution
  */
  void Cancel() override;
  
  /*!
  \brief Executes the action syncronously instead. Execute and wait for the result.
  */
...
private:
  ...
  /*!
  \brief Called always on action finish for cleanup
      Used for post-processing of the resolved item
  */
  void Finish();
