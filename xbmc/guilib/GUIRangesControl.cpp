...
CGUIRangesControl::CGUIRangesControl(int iParentID, int iControlID,
                                     ...
                                     const CTextureInfo& overlayTexture, int iInfo)
: CGUIControl(iParentID, iControlID, fPosX, fPosY, fWidth, fHeight),
  m_guiBackground(fPosX, fPosY, fWidth, fHeight, backGroundTexture),
  m_guiOverlay(fPosX, fPosY, fWidth, fHeight, overlayTexture),
  m_guiLowerTexture(fPosX, fPosY, fWidth, fHeight, lowerTexture),
  m_guiFillTexture(fPosX, fPosY, fWidth, fHeight, fillTexture),
  m_guiUpperTexture(fPosX, fPosY, fWidth, fHeight, upperTexture),
  m_iInfoCode(iInfo)
{
      ...
      if (values.size() % 2 == 0)
      {
        for (auto it = values.begin(); it != values.end();)
        {
          ...
          if (first <= second)
            ...
            CLog::Log(LOGERROR, "CGUIRangesControl::UpdateInfo - malformed ranges cvs string (end element must be larger or equal than start element)");
        }
      }
      else
        CLog::Log(LOGERROR, "CGUIRangesControl::UpdateInfo - malformed ranges cvs string (string must contain even number of elements)");
