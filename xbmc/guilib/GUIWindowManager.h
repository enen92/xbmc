...
public:
  ...
  void Initialize();
  void Add(std::shared_ptr<CGUIWindow> pWindow);
  void AddUniqueInstance(CGUIWindow *window);
  void AddCustomWindow(std::shared_ptr<CGUIWindow> pWindow);
  void Remove(int id);
  void Delete(int id);
...
private:
  ...
  void CloseWindowSync(std::shared_ptr<CGUIWindow> window, int nextWindowID = 0);
