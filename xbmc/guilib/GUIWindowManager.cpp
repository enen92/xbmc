...
void CGUIWindowManager::CreateWindows()
{
  ...
  Add(std::make_shared<CGUIDialogBusy>(CGUIDialogBusy::Passkey{}));
  ...
  Add(std::make_shared<CGUIWindowPVRTVProviders>());
  ...
  Add(std::make_shared<CGUIWindowPVRRadioProviders>());
  ...
  Add(std::make_shared<CGUIDialogVideoManagerVersions>());
  Add(std::make_shared<CGUIDialogVideoManagerExtras>());
  Add(std::make_shared<CGUIDialogSelect>(WINDOW_DIALOG_SELECT_VIDEO_STREAM));
  Add(std::make_shared<CGUIDialogSelect>(WINDOW_DIALOG_SELECT_AUDIO_STREAM));
  Add(std::make_shared<CGUIDialogSelect>(WINDOW_DIALOG_SELECT_SUBTITLE_STREAM));
  ...
  Add(std::make_shared<CGUIWindowFavourites>());
  ...
  Add(std::make_shared<GAME::CGUIAgentWindow>());
...
bool CGUIWindowManager::SendMessage(CGUIMessage& message)
{
  ...
  auto window = GetWindow(GetActiveWindow());
  if (window)
  {
    if (hasModalDialog)
    {
...
void CGUIWindowManager::AddUniqueInstance(std::shared_ptr<CGUIWindow> window)
{
  std::unique_lock lock(CServiceBroker::GetWinSystem()->GetGfxContext());
...
bool CGUIWindowManager::Add(std::shared_ptr<CGUIWindow> window)
{
  if (!window)
  {
  ...
  for (int id : window->GetIDRange())
  {
    ...
    m_mapWindows.insert(std::make_pair(id, window));
  }
  return true;
}

void CGUIWindowManager::AddCustomWindow(std::shared_ptr<CGUIWindow> window)
{
  if (!window)
    return;
...
void CGUIWindowManager::PreviousWindow()
{
  ...
  auto pCurrentWindow = GetWindow(currentWindow);
  if (!pCurrentWindow)
  ...
  if (m_windowHistory.size() < 2)
  {
    ...
    {
      CloseWindowSync(pCurrentWindow.get());
      ClearWindowHistory();
  ...
  auto pNewWindow = GetWindow(previousWindow);
  if (!pNewWindow)
  {
...
void CGUIWindowManager::ActivateWindow_Internal(int iWindowID, const std::vector<std::string>& params, bool swappingWindows, bool force /* = false */)
{
  ...
  auto pNewWindow = GetWindow(iWindowID);
  if (!pNewWindow)
  ...
  { // if we have a dialog, we do a DoModal() rather than activate the window
    if (!pNewWindow->IsDialogRunning())
    {
      ...
      static_cast<CGUIDialog*>(pNewWindow.get())->Open(!params.empty() ? params[0] : "");
  ...
  // deactivate any window
  int currentWindow = GetActiveWindow();
  auto pWindow = GetWindow(currentWindow);
  if (pWindow)
    CloseWindowSync(pWindow.get(), iWindowID);
  CServiceBroker::GetGUI()->GetInfoManager().GetInfoProviders().GetGUIControlsInfoProvider().SetNextWindow(WINDOW_INVALID);
...
bool CGUIWindowManager::SwitchToFullScreen(bool force /* = false */)
{
  ...
  {
    auto pDialog = GetWindow<CGUIDialogVideoInfo>(WINDOW_DIALOG_VIDEO_INFO);
    if (pDialog)
      pDialog->Close(true);
  }
  ...
  {
    auto pDialog = GetWindow<CGUIDialogVideoInfo>(WINDOW_DIALOG_MUSIC_INFO);
    if (pDialog)
      pDialog->Close(true);
  }
  ...
  {
    auto pDialog = GetWindow<CGUIDialogVideoInfo>(WINDOW_DIALOG_SONG_INFO);
    if (pDialog)
      pDialog->Close(true);
  }
...
void CGUIWindowManager::OnApplicationMessage(ThreadMessage* pMsg)
{
  ...
  case TMSG_GUI_DIALOG_OPEN:
  {
    ...
    {
      auto pDialog = GetWindow(pMsg->param1);
      if (pDialog)
        static_cast<CGUIDialog*>(pDialog.get())->Open(pMsg->strParam);
    }
  }
  break;
  ...
  case TMSG_GUI_ACTION:
  {
    if (pMsg->lpVoid)
    {
      ...
      {
        auto pWindow = GetWindow(pMsg->param1);
        if (pWindow)
  ...
  case TMSG_GUI_DIALOG_YESNO:
  {
    ...
    auto dialog = GetWindow(WINDOW_DIALOG_YES_NO);
    if (!dialog)
      return;

    if (pMsg->lpVoid)
      pMsg->SetResult(
          static_cast<CGUIDialogYesNo*>(dialog.get())
              ->ShowAndGetInput(*static_cast<HELPERS::DialogYesNoMessage*>(pMsg->lpVoid)));
    else
    {
      HELPERS::DialogYesNoMessage options;
      options.heading = pMsg->param1;
      options.text = pMsg->param2;
      pMsg->SetResult(static_cast<CGUIDialogYesNo*>(dialog.get())->ShowAndGetInput(options));
    }
  ...
  case TMSG_GUI_DIALOG_OK:
  {
    ...
    auto dialogOK = GetWindow(WINDOW_DIALOG_OK);
    if (!dialogOK)
      return;

    if (pMsg->lpVoid)
      static_cast<CGUIDialogOK*>(dialogOK.get())
          ->ShowAndGetInput(*static_cast<HELPERS::DialogOKMessage*>(pMsg->lpVoid));
    else
    {
      HELPERS::DialogOKMessage options;
      options.heading = pMsg->param1;
      options.text = pMsg->param2;
      static_cast<CGUIDialogOK*>(dialogOK.get())->ShowAndGetInput(options);
    }
    pMsg->SetResult(static_cast<int>(static_cast<CGUIDialogOK*>(dialogOK.get())->IsConfirmed()));
  }
  break;
  }
}
...
void CGUIWindowManager::Process(unsigned int currentTime)
{
  ...
  auto pWindow = GetWindow(GetActiveWindow());
  if (pWindow)
...
CGUIDialog* CGUIWindowManager::GetDialog(int id) const
{
  ...
  if (window && window->IsDialog())
    return dynamic_cast<CGUIDialog*>(window.get());
  return nullptr;
}

std::shared_ptr<CGUIWindow> CGUIWindowManager::GetWindow(int id) const
{
...
{
  auto topmost = GetWindow(GetTopmostDialog(modal, false));
