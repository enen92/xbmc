...
{
  std::shared_ptr<IDiscDriveHandler> platformDiscDriveHandler =
  ...
  DriveState driveState = platformDiscDriveHandler->GetDriveState(discPath);
  switch (driveState)
  {
    case DriveState::DRIVE_CLOSED_MEDIA_UNDEFINED:
      ...
      break;
    case DriveState::DRIVE_OPEN:
      m_TrayState = TrayState::TRAY_OPEN;
      break;
    default:
  ...
  if (m_TrayState == TrayState::TRAY_CLOSED_MEDIA_PRESENT)
  {
    if (m_LastTrayState != TrayState::TRAY_CLOSED_MEDIA_PRESENT)
    {
  ...
  else if (m_TrayState == TrayState::TRAY_CLOSED_NO_MEDIA)
  {
    if ((m_LastTrayState != TrayState::TRAY_CLOSED_NO_MEDIA) &&
        (m_LastTrayState != TrayState::TRAY_CLOSED_MEDIA_PRESENT))
    {
      m_LastTrayState = m_TrayState;
  ...
  else if (m_TrayState == TrayState::TRAY_OPEN)
  {
    if (m_LastTrayState != TrayState::TRAY_OPEN)
    {
      m_LastTrayState = m_TrayState;
...
{
  return m_DriveState == DriveState::DRIVE_READY;
}
