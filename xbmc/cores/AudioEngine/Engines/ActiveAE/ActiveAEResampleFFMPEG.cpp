                                   ...
                                   CAEChannelInfo *remapLayout, AEQuality quality, bool force_resample)
{
  ...
  av_channel_layout_from_mask(&dst_ch_layout, m_dst_chan_layout);
  av_channel_layout_from_mask(&src_ch_layout, m_src_chan_layout);

  int ret = swr_alloc_set_opts2(&m_pContext, &dst_ch_layout, m_dst_fmt, m_dst_rate, &src_ch_layout,
  ...
  else if (upmix && m_src_channels == 2 && m_dst_channels > 2)
  {
    ...
    {
      AVChannel out_chan = av_channel_layout_channel_from_index(&dst_ch_layout, out);
      switch(out_chan)
      {
