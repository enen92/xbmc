...
int CAEEncoderFFmpeg::Encode(uint8_t *in, int in_size, uint8_t *out, int out_size)
{
  ...
  try
  {
    ...
    int channel_num = m_CodecCtx->ch_layout.nb_channels;
#else
    frame->channel_layout = m_CodecCtx->channel_layout;
    frame->channels = m_CodecCtx->channels;
    int channel_num = m_CodecCtx->channels;
#endif

    avcodec_fill_audio_frame(frame, channel_num, m_CodecCtx->sample_fmt, in, in_size, 0);
    ...
    if (err < 0)
      throw FFMpegException("Error sending a frame for encoding (error '{}')", __func__,
    ...
    LIBAVUTIL_BUILD >= AV_VERSION_INT(57, 28, 100)
      ...
      {
        throw FFMpegException("Error during encoding (error '{}')", __func__,
                              FFMpegErrorToString(err));
      }
