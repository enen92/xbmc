...
void CDVDSubtitlesLibass::Configure()
{
  ...
  ass_set_fonts_dir(m_library, UTILS::FONT::FONTPATH::GetTranslatedFontPath(
                                   static_cast<std::string>(UTILS::FONT::FONTPATH::USER))
                                   .c_str());

  // Load additional fonts into Libass memory
  CFileItemList items;
  // Fonts from system directory
  XFILE::CDirectory::GetDirectory(static_cast<std::string>(UTILS::FONT::FONTPATH::SYSTEM), items,
                                  UTILS::FONT::SUPPORTED_EXTENSIONS_MASK,
                                  XFILE::DIR_FLAG_NO_FILE_DIRS | XFILE::DIR_FLAG_NO_FILE_INFO);
  // Temporary fonts
  XFILE::CDirectory::GetDirectory(static_cast<std::string>(UTILS::FONT::FONTPATH::TEMP), items,
                                  UTILS::FONT::SUPPORTED_EXTENSIONS_MASK,
                                  XFILE::DIR_FLAG_NO_FILE_DIRS | XFILE::DIR_FLAG_NO_FILE_INFO);
  for (const auto& item : items)
  {
    ...
    std::string fontFamily = FONT::GetFontFamily(filepath, buffer);
    if (fontFamily.empty())
      continue;
#if LIBASS_VERSION >= 0x01501000
    ass_add_font(m_library, fontFamily.c_str(), reinterpret_cast<const char*>(buffer.data()),
                 static_cast<int>(buffer.size()));
#else
    ass_add_font(m_library, const_cast<char*>(fontFamily.c_str()),
                 reinterpret_cast<char*>(buffer.data()), static_cast<int>(buffer.size()));
#endif
    if (StringUtils::CompareNoCase(item->GetLabel(), FONT::FONT_DEFAULT_FILENAME) == 0)
    {
      m_defaultFontFamilyName = fontFamily;
    }
  }
