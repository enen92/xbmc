...
#if LIBAVCODEC_BUILD >= AV_VERSION_INT(59, 37, 100) && \
    LIBAVUTIL_BUILD >= AV_VERSION_INT(57, 28, 100)
  int codec_channels = m_pCodecContext->ch_layout.nb_channels;
  uint64_t codec_channel_layout = m_pCodecContext->ch_layout.u.mask;
#else
  int codec_channels = m_pCodecContext->channels;
  uint64_t codec_channel_layout = m_pCodecContext->channel_layout;
#endif
  if (m_channels == codec_channels && m_layout == codec_channel_layout)
    return; //nothing to do here

  m_channels = codec_channels;
  m_layout = codec_channel_layout;

  int64_t layout;

  int bits = count_bits(codec_channel_layout);
  if (bits == codec_channels)
    layout = codec_channel_layout;
  else
  {
    CLog::Log(LOGINFO,
              ...
              codec_channels, bits);
    if (static_cast<int>(count_bits(m_hint_layout)) == codec_channels)
    ...
    LIBAVUTIL_BUILD >= AV_VERSION_INT(57, 28, 100)
      ...
      av_channel_layout_default(&def_layout, codec_channels);
