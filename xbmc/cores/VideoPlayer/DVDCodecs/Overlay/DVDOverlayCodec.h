...
#define OC_DONE 0x00000008 // the decoder has decoded the packet, no overlay will be provided because the previous one is still valid
...
protected:
   ...
   * \brief Adapts startTime, stopTIme from the subtitle stream (which is relative to stream pts)
   * so that it returns the absolute start and stop timestamps.
   * \param replace Will be set to true if the overlay should replace the former overlay
   * \param offset Optional - Offset will be added to start and stoptime
   */
