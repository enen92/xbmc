...
static int dir_read(DVD_DIR_H *dir, DVD_DIRENT *entry)
{
...
static DVD_DIR_H* dir_open(const char* rel_path)
{
 ...
 //{
  ...
  if (URIUtils::HasSlashAtEnd(strRelPath))
    URIUtils::RemoveSlashAtEnd(strRelPath);

  CLog::Log(LOGDEBUG, "CBlurayCallback - Opening dir {}", CURL::GetRedacted(strRelPath));

  SDirState *st = new SDirState();
  if (!CDirectory::GetDirectory(strRelPath, st->list, "", DIR_FLAG_DEFAULTS))
  {
    if (!CFile::Exists(strRelPath))
      CLog::Log(LOGDEBUG, "CBlurayCallback - Error opening dir! ({})",
                CURL::GetRedacted(strRelPath));
    delete st;
    return nullptr;
  }
...
bool CDVDInputStreamNavigator::Open()
{
  ...
  {
    //if (dvdnav_open(&m_dvdnav, path.c_str()) != DVDNAV_STATUS_OK)
    if (dvdnav_open_files(&m_dvdnav, &loggerCallback, path.c_str(), &m_dvdnav_dir_cb) != DVDNAV_STATUS_OK)
    {
